...
#include "version_variable.h"
  ...
  if (associated(Ice%sCS)) then ; if (associated(Ice%sCS%IST)) then
    ...
    call SIS_error(WARNING, "ice_model_init called ")
  ...
  if (slow_ice_PE) then
    ...
    call callTree_waypoint("SIS_initialize_fixed (ice_model_init)")

    call set_hor_grid(sG, param_file, global_indexing=global_indexing)
    call callTree_waypoint("set_hor_grid (ice_model_init)")

    call copy_dyngrid_to_SIS_horgrid(dG, sG)
    call destroy_dyn_horgrid(dG)
    call callTree_waypoint("destroy_dyn_horgrid (ice_model_init)")

  ! Allocate and register fields for restarts.

    call set_domain(sGD%mpp_domain)
    call callTree_waypoint("set_domain (ice_model_init)")
    ...
    call callTree_waypoint("ice_type_slow_reg_restarts (ice_model_init)")
    ...
    call callTree_waypoint("ice_state_register_res (ice_model_init)")

    call alloc_ocean_sfc_state(Ice%sCS%OSS, sHI, sIST%Cgrid_dyn)
    Ice%sCS%OSS%kmelt = kmelt
    call callTree_waypoint("alloc_ocean_sfc_state (ice_model_init)")
  ...
  ! restarts and advection.
    ...
    call callTree_waypoint("set_first_direction (ice_model_init)")

    call clone_MOM_domain(sGD, sG%domain_aux, symmetric=.false., &
                          domain_name="ice model aux")
    call callTree_waypoint("clone_MOM_domain aux(ice_model_init)")
    ...
    call callTree_waypoint("clone_MOM_domain nohalo (ice_model_init)")
  ...
  if (fast_ice_PE) then
    ...
    if (single_IST) then
      ...
      ! Set up the MOM_domain_type structures.
      call callTree_waypoint("Entering MOM_domains_init (ice_model_init)")
      ...
      fGD => Ice%fCS%G%Domain
      call callTree_waypoint("MOM_domains_init (ice_model_init)")
